...
const openai = new OpenAIApi(); //Instance of OpenAI

const newRecipe = async function (data) {
  console.log("Recipe Generator Middleware - newRecipe");
  if (config.mockData) {
    //for not allways be using OpenAI, it's expensive
    console.log("@@@@@@@@@@@@@ MOCK DATA!!! @@@@@@@@@@@@@");
    const recipeMock = {
      //this recipe was already generated with OpenAI - this is an expected result!
      ...
      instructions: [
        ...
        {
          instruction:
            "In a separate bowl, beat the eggs and slowly drizzle into the soup while stirring constantly.",
        },
        {
          instruction:
            "Sprinkle Parmesan cheese over the soup and stir to combine.",
        },
        {
      ...
      notes:
        "This Hearty Pasta Soup is a satisfying and nutritious meal that is sure to please your taste buds. The blend of flavors from the fresh ingredients and spices creates a comforting dish perfect for any time of the year.",
      allergenInformation: [
        { name: "Dairy", info: "Contains Parmesan cheese" },
      ],
      substitutions: [
  ...
  let recipe = recipeModel.newDefaultRecipe(); //new empty recipe base

  recipe = await buildBase(data, recipe); //Title, ingredients description
  data.recipeName = recipe.recipeName; //add more info to the data to be fed into OpenAi API
  data.description = recipe.description;

  recipe = await buildMainInfo(data, recipe);
  data.instructions = recipe.instructions; //add more info to the data to be fed into OpenAi API

  recipe = await buildSecondaryInfo(data, recipe);

  recipe = await buildTags(recipe); //from generated info get the tags

  return recipe;
};

const buildBase = async function (data, base) {
  console.log("Recipe Generator Middleware - buildBase");

  let cleanDataString = generateReadableString(data);
  //Setting up conversation for Open AI
  const setupPrompt = {
    //Explain
    role: "system",
    content:
  ...
  const response = JSON.parse(request.choices[0].message.content); //get response
...
const buildMainInfo = async function (data, base) {
  console.log("Recipe Generator Middleware - buildMainInfo");
  let cleanDataString = generateReadableString(data);

  const setupPrompt = {
    //promp setup as before ^
    role: "system",
    content:
  ...
  //Response
  const response = JSON.parse(request.choices[0].message.content); //get recipe
...
const buildSecondaryInfo = async function (data, base) {
  console.log("Recipe Generator Middleware - buildSecondaryInfo");
  let cleanDataString = generateReadableString(data);
  ...
  //Response
  const response = JSON.parse(request.choices[0].message.content); //get info
...
const buildTags = async function (base) {
  // Extract meal type, cuisine type, and ingredients
  console.log("Recipe Generator Middleware - buildTags");
  const { mealType, cuisine, ingredients } = base;

  // Add meal type and cuisine type as tags - lower case
