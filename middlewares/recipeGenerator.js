...
const buildBase = async function (data, base) {
  ...
  let cleanDataString = generateReadableString(data);   //Setting up conversation for Open AI

  const setupPrompt = {  //Explain
    role: "system",
    content:
  ...
  const instructionPrompt = {  //Instruct
    role: "system",
    content:
  ...
  const request = await openai.chat.completions.create({  //send to OpenAI API
    model: "gpt-3.5-turbo",
...
const buildMainInfo = async function (data, base) {
  ...
  const request = await openai.chat.completions.create({ //get recipe
    model: "gpt-3.5-turbo",
    response_format: { type: "json_object" },
  ...
  //Set new values  //Set new values if they exist
...
const buildSecondaryInfo = async function (data, base) { console.log("Recipe Generator Middleware - buildSecondaryInfo");
  //Request -- TODO promp engeneering
  let cleanDataString = generateReadableString(data);
